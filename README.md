# 👋 Hey! I'm **Nibha Kumari**

<div align="center">
  
![Typing](https://readme-typing-svg.herokuapp.com?font=Fira+Code&size=22&duration=4000&pause=1000&color=00D4FF&center=true&vCenter=true&width=600&lines=AI-Augmented+Full+Stack+Developer;Generative+AI+Enthusiast;Building+Scalable+Solutions)

**Full Stack Developer** | **Generative AI** | **Backend Engineer**

📍 India | 🎓 B.Tech CSE, Bennett University (2023-2027)

</div>

---

## 💡 About Me

AI-Augmented Full Stack Developer passionate about building intelligent, scalable web applications. Specialized in **MERN Stack**, **Generative AI**, and **backend optimization** with proven experience delivering production-ready solutions.

**Key Focus:**
- 🤖 AI-powered applications & LLM integration
- 💻 Scalable full-stack systems & REST APIs  
- 📊 Database optimization & system design
- 🚀 Real-time applications & performance tuning

---

## 🛠️ Tech Stack

**Languages:** JavaScript | Python | Java | SQL

**Frontend:** React | React Native | HTML5 | CSS3

**Backend:** Node.js | Express.js | REST APIs | Socket.io

**Databases:** MongoDB | MySQL

**AI/ML:** ChatGPT | Claude | Gemini | GitHub Copilot | Prompt Engineering | NLP

**Automation & Tools:** Playwright | n8n | Git | GitHub | Docker | Postman | VS Code | Power BI

---

## 🚀 Featured Projects

### **BU AI Chatbot** – Generative AI University Assistant
- Handles 100+ academic & administrative queries
- NLP-based conversational workflows with prompt engineering
- [Repository](https://github.com/Nibha840/Bennett_ChatBotDeploy)

### **CineBook** – Full Stack Movie Booking Platform  
- MERN stack | 12+ REST APIs | JWT authentication
- Razorpay payment integration | Real-time seat management
- 100% booking accuracy | 99.9% reliability
- [Repository](https://github.com/Nibha840/movie_bookingRNatve)

### **GharPay CRM** – Enterprise Lead Management
- Manages 1000+ leads with role-based access control
- Dashboard & analytics system | 30% efficiency improvement
- [Repository](https://github.com/Nibha840/GharPay-Proj)

### **Zerodha Trading Clone** – Stock Trading Dashboard
- MERN architecture | Portfolio management | Real-time data visualization
- [Repository](https://github.com/Nibha840/AllZerodha)

### **RFQ Auction System** – Real-Time Bidding Platform
- Socket.io live updates | Concurrent bidding support
- 90% database load reduction through query optimization
- [Repository](https://github.com/Nibha840/RFQ-British-Auction-System)

### **AI Research Agent** – Autonomous Web Research Platform
- Processes 50+ web sources → PDF reports in <2 minutes
- Playwright web automation + Gemini API for content extraction & synthesis
- [Repository](https://github.com/Nibha840/AI-Research-Agent)

---

## 🏆 Highlights

| Achievement | Details |
|---|---|
| **Microsoft Innovate Hackathon** | Top 10 Finalist |
| **Problem Solving** | 300+ DSA problems solved |
| **Full Stack Projects** | 6+ production-ready applications |
| **DB Optimization** | 90% load reduction, 30% query improvement |
| **SQL Optimization** | 50+ queries optimized |

---

## 🎓 Certifications

✅ IBM Artificial Intelligence | ✅ UC San Diego Data Structures | ✅ Coursera Web Development

---

## 🔬 Research

**Comparative Analysis of Deep Learning Architectures for Time Series Forecasting**
- Evaluated: GRU, LSTM, TCN, Transformer models
- Best Model: GRU with MAPE of 3.54%

---

## 🤝 Connect With Me

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=flat&logo=linkedin)](https://www.linkedin.com/in/nibha-kumari-882b4a24b/)
[![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat&logo=github)](https://github.com/Nibha840)
[![Email](https://img.shields.io/badge/Email-D14836?style=flat&logo=gmail)](mailto:nibhakumari840@gmail.com)
[![Twitter](https://img.shields.io/badge/Twitter-1DA1F2?style=flat&logo=twitter)](https://twitter.com/NibhaKumari)

**Open to:** Full-time roles | Internships | Freelance projects | Open-source contributions

---

<div align="center">

> *"Building intelligent solutions through clean code and continuous learning."*

![Profile Views](https://komarev.com/ghpvc/?username=Nibha840&color=00D4FF&style=flat-square)

</div>